...
const VoiceFilterStudio = ({ mode }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'filter') {
        data = await processAudio(file, selectedOption, selectedQuality.format, selectedQuality.bitrate);
        setResult({ url: `${BASE}${data.downloadUrl || `/api/audio/download/${data.filename}`}` });
      } 
      else if (mode === 'spleeter') {
        data = await separateAudio(file, selectedOption);
        const path = selectedOption === 'vocals' ? data.files.vocals : data.files.accompaniment;
        ...
        setResult({ url: `${BASE}${data.files.video}`, assUrl: `${BASE}${data.files.subtitles_ass}` });
      }
    } catch (err) {
  ...
  const commonProps = {
    file, setFile, result, isProcessing, onSubmit: handleSubmit,
    error, selectedOption, setSelectedOption, fileInputRef,
    handleFileChange: (e) => {
